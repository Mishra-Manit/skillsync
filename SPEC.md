...
- **gh CLI** — required and must be authenticated (`gh auth login`). Used for repo creation, collaborator invites, and cloning private repos. Install at https://cli.github.com.
...
| Layer | Choice | Reason |
|-------|--------|--------|
| Language | TypeScript | Team already knows JS/TS; target users all have Node |
| Runtime | Node.js 20+ | Universal on developer machines |
| CLI framework | Commander.js | Command routing (`create`, `join`, `sync`) |
| Interactive UI | @clack/prompts | Modern connected-line wizard UI; used by create-astro, create-svelte |
| Git operations | simple-git | Node wrapper; no shell-out needed |
...
| Colors | picocolors | Same as Clack internals; avoids ESM/CJS headaches |
| Distribution | npm / npx | Zero-install: `npx skillsync join org/repo` just works |
...
Flow:
1. Prompt: team name (becomes the GitHub repo name)
2. Prompt: GitHub org (optional — uses personal account if blank)
3. Detect `gh` CLI — if authenticated, creates the private repo automatically; otherwise init locally and tell the user to push
4. Seed the repo with a starter `skills/` and `agents/` directory structure, plus an example SKILL.md
5. Prompt: invite teammates (comma-separated emails or GitHub usernames)
   - Uses `gh` CLI to send collaborator invites
   - Org repos support email invites; personal repos require GitHub usernames
6. Prompt: import existing skills (see Import Flow below)
7. Commit + push everything
8. Print the `join` command for the lead to share with teammates

```
$ npx skillsync create
  ...
  Share this with your team:
    ...
    npx skillsync join acme/acme-skills
```
...
2. Read `skillsync.toml` from the repo for target preferences
...
- Runs as a detached Node process, PID stored at `~/.skillsync/daemon.pid`
...
[sync]
interval = 60        # seconds (daemon poll interval)
strategy = "auto"    # auto | manual | pull-only
...
skillsync/
  src/
    ...
    lib/
      ...
      config.ts      # read/write ~/.skillsync/config.toml
...
v0 success metric: a team lead can go from `npx skillsync create` to a teammate successfully running `npx skillsync join` and having skills in Claude Code in under 60 seconds (excluding GitHub invite accept time).
...
**Why Commander + Clack instead of Ink?** Ink (React for the terminal) is powerful but heavyweight for a setup wizard. Clack gives the clean, modern interactive UI we need with a simple API and no React complexity.
...
**Why `npx` distribution?** Zero install friction. The target user (`npx skillsync join org/repo`) never needs to install anything globally. Works immediately after receiving the join command from their team lead.
...
Get a working CLI binary with no real logic yet. Goal: `npx skillsync --help` works end-to-end.

- [ ] Initialize npm package (`package.json`, `tsconfig.json`, `.gitignore`)
- [ ] Install all v0 dependencies (`@clack/prompts`, `commander`, `simple-git`, `gray-matter`, `picocolors`, `zod`)
- [ ] Set up TypeScript build (`tsc`, `outDir: dist/`, `strict: true`)
- [ ] Create `src/index.ts` with Commander — register `create`, `join`, `sync`, `status`, `import` as stub commands
- [ ] Add `bin` field to `package.json`, verify `node dist/index.js --help` prints the command list
- [ ] Add `npm run build`, `npm run dev`, `npm run typecheck` scripts
...
**`src/lib/config.ts`**
- [ ] Define `Config` type and Zod schema for `~/.skillsync/config.toml`
- [ ] `readConfig()` — parse config file, return typed object or `null` if not found
- [ ] `writeConfig(config)` — write config file, create `~/.skillsync/` directory if needed

**`src/lib/github.ts`**
- [ ] `detectGh()` — shell out to `gh auth status`, return `{ available, username }` or throw with helpful message if not installed/authenticated
...
- [ ] Call `detectGh()` — exit with error if gh is not authenticated
- [ ] Call `createRepo(name, org)` — create private GitHub repo and clone it locally to `~/.skillsync/store/`
- [ ] Seed the cloned repo: create `skills/`, `agents/` directories, write a starter `SKILL.md` example, write `skillsync.toml`, write `README.md` with the join command
...
- [ ] Write `~/.skillsync/config.toml` with repo slug and team name
...
- [ ] Call `detectGh()` — exit with error if not authenticated
- [ ] Clone repo to `~/.skillsync/store/` using `cloneRepo()`
- [ ] Read `skillsync.toml` from the cloned repo to get target preferences
- [ ] Write `~/.skillsync/config.toml` locally
...
- [ ] Error messages: every thrown error should produce a Clack-styled `p.log.error()` with a clear next step, never a raw stack trace
- [ ] Validate `gh` is installed at the start of every command that needs it, not mid-flow
...
- [ ] Verify `npx skillsync --help` works from a clean machine with no prior install
