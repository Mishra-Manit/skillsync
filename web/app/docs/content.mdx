...
- **Bun 1.0+** (Node.js is not supported)
...
Without a name, shows an interactive multiselect of all linked items — nothing is pre-selected.

### leave

Leave a joined team repo and clean up everything locally.
...
Removes all symlinks for that repo, restores any backed-up files, deletes the local store directory, and removes it from your config.

At the end, you'll be asked whether to also delete the GitHub repo itself. Choosing no leaves the repo intact for your teammates.
